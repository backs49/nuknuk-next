...
| `customer_email` | TEXT | 이메일 (선택, 토스 결제 및 주문 확인 메일용) |
...
| `default_shipping_fee` | INTEGER | 기본 배송비 (원, 기본값 0) |

### `order_items` FK 참고

`menu_item_id`는 FK 제약 조건 없음. 커스텀 상품(링크 결제 시 사장님이 직접 입력)의 경우 null이 될 수 있으므로 참조 무결성 대신 상품명 스냅샷(`name`)으로 기록을 보존한다.

### 인덱스

- `idx_orders_status` — `orders(status)`
- `idx_orders_paid_at` — `orders(paid_at)`
- `idx_orders_channel` — `orders(channel)`
- `idx_orders_order_number` — `orders(order_number)` (UNIQUE 제약으로 자동 생성)
- `idx_order_items_order_id` — `order_items(order_id)`

### 주문번호 생성

DB 함수로 생성: `NUK-YYYYMMDD-NNN` 형식. Supabase SQL 함수에서 해당 날짜의 최대 시퀀스를 조회 후 +1. `SELECT ... FOR UPDATE`로 동시성 제어.

### RLS 정책

- `orders`: SELECT은 `order_number` 기반 조건부 공개 (고객은 자기 주문만 조회). INSERT/UPDATE/DELETE는 서비스 롤만.
- `order_items`: SELECT은 해당 주문의 `order_number`를 아는 경우만. INSERT/UPDATE/DELETE는 서비스 롤만.
- 고객 주문 생성은 공개 INSERT가 아닌 **API 엔드포인트(`POST /api/orders`)에서 서비스 롤 키로 처리**하여 서버 측 검증 후 저장.
...
pending (대기) → paid (결제완료) → confirmed (확정) → completed (완료)
                    ↓                    ↓                    ↓
                cancelled (취소)    cancelled (취소)    refunded (환불)
                    ↓
                refunded (환불)
```
...
- 커스텀: 시작일 ~ 종료일 날짜 피커
- **핵심 지표 카드(상단)**는 항상 고정 기간 표시 (오늘/이번 주/이번 달)
- **차트 및 분석 테이블(중단/하단)**은 선택한 기간 필터에 따라 변경
...
새로운 주문 플로우로 대체되므로 `/order` 페이지로 리다이렉트 처리.

### 알려진 제한사항

- 고객 직접 주문은 단일 상품 단위. 여러 상품을 함께 주문하려면 각각 별도 주문. 멀티 상품 장바구니는 향후 확장 사항.
...
| GET | `/api/orders/[orderNumber]` | 없음 | 주문 조회 (결제 링크 페이지용, order_number 기반) |
...
| 메서드 | 경로 | 인증 | 용도 |
|--------|------|------|------|
| GET | `/api/admin/dashboard/summary` | 관리자 | 핵심 지표 (오늘/주/월 매출, 주문 건수) — 항상 고정 기간 |
| GET | `/api/admin/dashboard/sales` | 관리자 | 매출 추이 (일별/월별, 기간 파라미터: `from`, `to`, `granularity=daily\|monthly`) |
| GET | `/api/admin/dashboard/breakdown` | 관리자 | 분석 데이터 (`type=category\|product\|channel`, 기간 파라미터) |
...
`POST /api/payments/confirm`에서 토스 API 호출 전에 반드시 DB의 `orders.total_amount`와 클라이언트가 보낸 `amount`를 비교 검증한다. 불일치 시 결제 거부.

### API 키 관리

토스페이먼츠 클라이언트 키와 시크릿 키를 환경변수로 관리:
- `NEXT_PUBLIC_TOSS_CLIENT_KEY` — 클라이언트 위젯용
- `TOSS_SECRET_KEY` — 서버 결제 확인용

기존 코드의 하드코딩된 테스트 키를 환경변수로 교체.

### 주문 API 보호

- `POST /api/orders` (고객 주문 생성): 서버 측 입력값 검증 (가격은 DB의 메뉴 가격과 대조, 수량 범위 제한). 서비스 롤 키로 DB 접근.
- Rate limiting: 동일 IP에서 짧은 시간 내 과도한 주문 생성 방지 (미들웨어 레벨).
- 공개 주문 조회 API(`GET /api/orders/[orderNumber]`)는 주문번호를 알아야만 접근 가능하며, 민감 정보(admin_memo 등)는 응답에서 제외.

### 결제 링크 만료

링크 결제 주문(`channel: 'link'`)은 생성 후 72시간 경과 시 만료 처리. `pending` 상태인 주문의 `created_at` 기준으로 만료 판단 (결제 페이지에서 체크).
...
| 변수 | 용도 |
|------|------|
| `NEXT_PUBLIC_TOSS_CLIENT_KEY` | 토스 클라이언트 위젯 키 |
| `TOSS_SECRET_KEY` | 토스 서버 시크릿 키 |
